...
% https://ru.wikipedia.org/wiki/Параметрическое_представление
...
% y = sqrt(sqrt(4 a^2 x^2 + b^4) - a^2 - x^2)
 

% Parametric: Sqrt[M/2] * {Cos[t], Sin[t]} where M is 2 *a^2* Cos[2 t] + 2 *Sqrt[(-a^4+b^4) + a^4 Cos[2* t]^2], 0 < t ≤ 2 π, and a < b.
% (This parametrization does not generate the complete curve when a > b.) derive parametric form of Cassini oval.

    

%fx = @(t, b, a) (b^4 - t.^4) / (4 * a * t.^2); <- исходная функция
fx = @(t, b, a) (b.^4 - t.^4) ./ (4 * a .* t.^2); 
fy_plus = @(t, b, a) t.*sqrt(1 - ((b.^4 - 4*a.^2.*t.^2 - t.^4)/(4*a*t.^3)).^2);
fy_minus = @(t, b, a) -t.*sqrt(1 - ((b.^4 - 4*a.^2.*t.^2 - t.^4)/(4*a*t.^3)).^2);
a = 0.1; b = 10;
tmin = -a + sqrt(a^2 + b^2); tmax = a + sqrt(a^2 + b^2); dt = .01;
s1 = .14; s2 = .14; r = 0;
...
fprintf('test_fy_plus:')
disp(testY);

testY_minus = fy_minus(tmin:dt:tmax, b, a);
fprintf('test_fy_minus:')
disp(testY_minus);
...
% https://www.researchgate.net/figure/A-family-of-Cassini-Ovals_fig1_329186459
[x_retarr, y_retarr] = add_cassini_points_to_arrays(0, 1, -10, 10);
x_arr = horzcat(x_arr, x_retarr);
y_arr = horzcat(y_arr, y_retarr);

[x_retarr, y_retarr] = add_cassini_points_to_arrays(1.4, 2, -10, 10);
x_arr = horzcat(x_arr, x_retarr);
y_arr = horzcat(y_arr, y_retarr);

[x_retarr, y_retarr] = add_cassini_points_to_arrays(2, 2, -10, 10);
x_arr = horzcat(x_arr, x_retarr);
y_arr = horzcat(y_arr, y_retarr);

for i = 1:length(a_b_variants)
...
axis_data{12} = [-10 10 -5 5];
data_type_names{12} = 'Oval Kassini';

%class1.type = 4;
%class1.par1 = {{2, x_arr, y_arr}};
%class1.par2 = {{2, [0, 0]}};
%class1.par3 = {1};

%class_data{12} = {class1, class1, class1};
...
function [y] = get_cassini_y(x, a, b)
    y = sqrt(sqrt(4*a^2*x^2 + b^4) - a^2 - x^2);
end

function [y] = get_cassini_minus_y(x, a, b)
    y = -sqrt(sqrt(4*a^2*x^2 + b^4) - a^2 - x^2);
end

function [x_arr, y_arr] = add_cassini_points_to_arrays(a, b, startx, endx)
    x_arr = [];
    y_arr = [];
    for x = startx:0.01:endx
        val = get_cassini_y(x, a, b);
        if isreal(val)
            x_arr(end+1) = x;
            y_arr(end+1) = val;
        end
    end
    for x = endx:-0.01:startx
        val = get_cassini_minus_y(x, a, b);
        if isreal(val)
            x_arr(end+1) = x;
            y_arr(end+1) = val;
        end
