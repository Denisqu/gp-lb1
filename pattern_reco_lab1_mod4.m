function pattern_reco_lab1_mod4 %(C, N, theta, centers, sigmas, p_w, axis_rect)
...
global parDistrType_data nCompPar_data ;
...
global NN_arch_type NN_out_type NN_train_alg NN_hidden_neurons;
...
learners.net = create_neural_network(NN_arch_type, NN_out_type, NN_hidden_neurons); learners.svms = {}; learners.tree = [];
...
%                    end;
                ...
                elseif h2 == 5 && is_error == 0
                    prompt = {'Enter deltaT3 value:','Enter Np_calc_length value:', 'Enter k_curve value:'};
                    dlg_title = 'Value:';
                    num_lines = 1;
...
elseif classType == 6
    [p_mle, c_levels, learners.net] = get_nn_dens_est(x_all, dataTrain, groupsTrain,classDiap, learners.net, NN_arch_type, NN_out_type, NN_train_alg, NN_hidden_neurons, params.net_need_rebuild , params.net_need_retrain);
...
for i = 1:length(classDiap)
        ...
        for d = 1:size(x_allc,2)
            if classType == 2
                ...
                elseif parDistrType(c) == 2   %GMM
                    [p_mle_d, c_levels_d ] = get_gmm_dens_est(x_allc(:,d), xtrainc(:,d), nCompPar(c));
                end;
    ...
    elseif isNaiveBayes(c) == 2
        if classType == 2 
            ...
            elseif parDistrType(c) == 2 % GMM
                [p_mle(i,:), c_levels(i)] = get_gmm_dens_est(x_allc, xtrainc, nCompPar(c));
            end;
        elseif classType == 3   % KDE
...
function [p_mle,c_levels] = get_gmm_dens_est(x, xTrain, nComp)
k = 3;
k_mod = 0.3;
if size(x,2) == 1
    %obj = gmdistribution.fit(xTrain, nComp);
    obj = gmdistribution.fit(xTrain, nComp, 'Regularize', 0.1);

    p_mle = pdf(obj, x)';
    c_levels = 0;
    for nc = 1:nComp
        c_levels = c_levels + obj.ComponentProportion(nc)/sqrt(obj.Sigma(:,:,nc));
    end;
    ...
    c_levels = 0;
    for nc = 1:nComp
        c_levels = c_levels + obj.ComponentProportion(nc)/det(obj.Sigma(:,:,nc)^0.5);
    end;
...
function    [p_mle, c_levels, net] = get_nn_dens_est(x_all, dataTrain, groupsTrain, classDiap, net, NN_arch_type, NN_out_type, NN_train_alg, NN_hidden_neurons, net_need_rebuild , net_need_retrain)
if net_need_rebuild
    net = create_neural_network(NN_arch_type, NN_out_type, NN_hidden_neurons);
end;
if net_need_retrain
    net = train_neural_network(net, dataTrain, groupsTrain, NN_out_type, NN_train_alg);
end;

p_mle = sim_neural_network(net, x_all, NN_out_type);
...
function net = train_neural_network(net, dataTrain, groupsTrain, NN_out_type, NN_train_alg)
trainFcns = { 'traingd', 'traingdm', 'traingda', 'traingdx', 'trainrp', 'traincgf', 'traincgb', 'traincgp', 'trainscg', 'trainlm', 'trainbfg', 'trainoss', 'trainbr'};
net.trainFcn = trainFcns{NN_train_alg};
C = max(groupsTrain);
...
function y = sim_neural_network(net, dataAll, NN_out_type)
if NN_out_type == 1 || NN_out_type == 2
    y = sim(net, dataAll');
end;
end
...
global classType parDistrType_data nCompPar_data isNaiveBayes_data;
...
%% functions basic callbacks
    ...
    function btnNN_train_start_callback(edit, events)
        if dataPR_copy.params.net_need_rebuild
            dataPR_copy.learners.net = create_neural_network(NN_arch_type_copy, NN_out_type_copy, NN_hidden_neurons_copy);
            % Rebuild network
            dataPR_copy.params.net_need_rebuild = 0;
        end;
        % Train network
        dataPR_copy.learners.net = init(dataPR_copy.learners.net);
        dataPR_copy.learners.net = train_neural_network(dataPR_copy.learners.net, dataPR_copy.dataTrain, dataPR_copy.groupsTrain, NN_out_type_copy, NN_train_alg_copy);
...
%% functions shared callbacks
    function popups_callback(cb, event)
        ...
        if ~get(checkboxNCompParAll, 'Value')
            ...
            set(editNCompPar, 'String', num2str(nCompPar_data_copy{2}));
        end;
        ...
        controls1 = {{popupParDistrType, txtposParDistrType },...
            ...
            {txtNNPars1, popupNN_ArchType, txtNN_OutType, popupNN_OutType, txtNNPars2, editNN_hidden_neurons, txtNN_train_alg, popupNN_train_alg, btnNN_train_start, btnNN_view, btnNN_gensim}...
...
%            set(popupsClass{i}, 'Enable', on_off{2-(vals(i) & (get(checksAll{i}, 'Value') ~= 1))});
    ...
    function checkboxes_callback(cb, event)
        ...
        v = get(checkboxNCompParAll, 'Value'); set(popupNCompParClass, 'Enable', on_off{v+1}); nCompPar_data_copy{1} = v;
    ...
    function btnSave_callback(dlg,event)
        ...
        vars = {'N_data_copy', 'Ntrain_data_copy', 'p_w_copy', 'theta_copy', 'C_matr_copy', 'n_class_2_copy', 'pos_class_copy', 'bayes_type_copy', 'classType_copy', 'data_type_copy',...
            'isNaiveBayes_data_copy', 'parDistrType_data_copy', 'nCompPar_data_copy', 'normType_data_copy', 'parzenType_data_copy', 'smoothPar_data_copy', 'smoothParCalc_data_copy',...
            ...
            'NN_arch_type_copy', 'NN_out_type_copy', 'NN_train_alg_copy', 'NN_hidden_neurons_copy',...
...
%% functions - get strings
    ...
    function str_ar = get_NN_out_types
        str_ar = {'sigmoid', 'softmax'};
    end
    function str_ar = get_NN_train_types
        str_ar = { 'traingd', 'traingdm', 'traingda', 'traingdx', 'trainrp', 'traincgf', 'traincgb', 'traincgp', 'trainscg', 'trainlm', 'trainbfg', 'trainoss', 'trainbr'};
    end
end
%% Other Dialogs
